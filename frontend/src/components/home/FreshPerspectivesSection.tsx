...
export function FreshPerspectivesSection({ data }: FreshPerspectivesSectionProps) {
    ...
    <section className="bg-principal py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {resaltar.map((item) => {
            const { id, resalto, titulo: itemTitulo, subtitulo: itemSubtitulo, boton } = item;
            const { texto, link } = boton;

            return (
              <div key={id} className="text-center">
                <div className="text-secundario mb-4 text-6xl font-bold">{resalto}</div>
